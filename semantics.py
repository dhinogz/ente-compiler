...
def validate_semantics(left_operand_type, right_operand_type, operator) -> str:
    ...
    try:
        result_type = SEMANTIC_CUBE[left_operand_type][right_operand_type][operator_symbol]
        print(f"{left_operand_type} {operator_symbol} {right_operand_type} -> {result_type}")
        return result_type
    except KeyError:
        print(f"Error: Invalid operation {left_operand_type} {operator_symbol} {right_operand_type}")
        return "error"
